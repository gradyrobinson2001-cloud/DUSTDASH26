...
export function useRota(weekStart) {
  ...
  const upsertRota = async (teamId, updates) => {
    ...
    const record = { week_start: weekStart, team_id: teamId, ...updates };
    const { error } = await supabase.from('rota').upsert(record, { onConflict: 'week_start,team_id' });
    if (error) throw error;
    setRotas(prev => {
      const idx = prev.findIndex(r => r.team_id === teamId && r.week_start === weekStart);
      if (idx >= 0) { const updated = [...prev]; updated[idx] = { ...updated[idx], ...record }; return updated; }
      return [...prev, record];
    });
  };
