...
export function useScheduledJobs({ staffId } = {}) {
  ...
  const addJob = async (j) => {
    ...
    setScheduledJobs(prev => [...prev, data]);
    return data;
  };
  ...
  const updateJob = async (id, updates) => {
    ...
    if (error) throw error;
    setScheduledJobs(prev => prev.map(j => j.id === id ? { ...j, ...updates } : j));
  };
  ...
  const bulkUpsertJobs = async (jobs) => {
    ...
    if (error) throw error;
    // Merge upserted jobs into local state
    setScheduledJobs(prev => {
      const map = new Map(prev.map(j => [j.id, j]));
      jobs.forEach(j => map.set(j.id, { ...(map.get(j.id) || {}), ...j }));
