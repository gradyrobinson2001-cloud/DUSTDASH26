...
export function usePayroll(weekStart) {
  ...
  const savePayrollRecord = async (record) => {
    ...
    const { data, error } = await supabase.from('payroll_records').upsert(record, { onConflict: 'staff_id,week_start' }).select('*, profiles(full_name)').single();
    if (error) throw error;
    setPayrollRecords(prev => {
      const idx = prev.findIndex(r => r.staff_id === data.staff_id && r.week_start === data.week_start);
